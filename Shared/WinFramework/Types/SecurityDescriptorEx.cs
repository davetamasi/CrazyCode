...
using fr = Tamasi.Shared.WinFramework;
...
namespace Tamasi.Shared.WinFramework.Types
{
	...
	public class SecurityDescriptor
	{
		...
		{
			Match m = fr.CommonRegex.SddlStringRegex.Match( sddlString.ToString() );
		...
		private static readonly string[] wellKnownSids = new string[]
	{
		...
		private static extern Boolean ConvertStringSidToSid( [MarshalAs( UnmanagedType.LPWStr )] string StringSid, out IntPtr Sid );

		[DllImport( "Advapi32.dll" )]
		private static extern Boolean ConvertSidToStringSid( IntPtr Sid, out IntPtr StringSid );

		[DllImport( "Advapi32.dll" )]
		private static extern UInt32 GetSidLengthRequired( UInt16 nSubAuthorityCount );

		[DllImport( "Advapi32.dll" )]
		private static extern Boolean CreateWellKnownSid( Int32 WellKnownSidType, IntPtr DomainSid, IntPtr pSid, ref UInt32 cbSid );

		[DllImport( "Advapi32.dll" )]
		private static extern Boolean LookupAccountSid( [MarshalAs( UnmanagedType.LPTStr )]string lpSystemName,
														IntPtr lpSid,
														IntPtr lpName,
														ref UInt32 cchName,
		...
		[DllImport( "Advapi32.dll" )]
		private static extern Int32 LsaOpenPolicy( IntPtr SystemName,
												ref LSA_OBJECT_ATTRIBUTES ObjectAttributes,
												ACCESS_MASK DesiredAccess,
												out IntPtr PolicyHandle );

		[DllImport( "Advapi32.dll" )]
		private static extern UInt32 LsaNtStatusToWinError( Int32 Status );

		[DllImport( "Advapi32.dll" )]
		private static extern Int32 LsaLookupNames2( IntPtr PolicyHandle,
													UInt32 Flags,
													UInt32 Count,
		...
		[DllImport( "Advapi32.dll" )]
		private static extern Int32 LsaFreeMemory( IntPtr Buffer );
		...
		private static extern UInt32 FormatMessage( UInt32 dwFlags, IntPtr lpSource, UInt32 dwMessageId, UInt32 dwLanguageId, [MarshalAs( UnmanagedType.LPTStr )] ref string lpBuffer, Int32 nSize, IntPtr[] Arguments );

		private const UInt16 SID_MAX_SUB_AUTHORITIES = 15;
		...
		public string WellKnownSidAbbreviations
		{
			get
			{
				if( this.IsWellKnownSid && !String.IsNullOrEmpty( SecurityIdentity.wellKnownSidAbbreviations[ ( Int32 )this.wellKnownSidType ] ) )
				{
					return SecurityIdentity.wellKnownSidAbbreviations[ ( Int32 )this.wellKnownSidType ];
				}
		...
		private void SetFromName( string name )
		{
			...
			Int32 status = SecurityIdentity.LsaOpenPolicy( IntPtr.Zero, ref attribs, ACCESS_MASK.POLICY_LOOKUP_NAMES, out handle );
		...
		private void SetFromWellKnownSidType( WellKnownSidType sidType )
		{
			...
			UInt32 size = SecurityIdentity.GetSidLengthRequired( SecurityIdentity.SID_MAX_SUB_AUTHORITIES );
			...
			try
			{
				...
				if( !SecurityIdentity.CreateWellKnownSid( ( Int32 )sidType, IntPtr.Zero, sidStruct, ref size ) )
				{
					throw new ExternalException( "Unable to Get Well Known SID" );
				}
