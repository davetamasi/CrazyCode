...
{
	public static class ObjectCaster<T> where T : class
	{
		/// <summary>
		...
		public static T As( object o )
		{
			...
			catch( System.Exception ex )
			{
				...
				(
					"Could not cast the instance of {0} to {1}. Check the inner exception for details.",
					sourceType,
					interfaceType,
					ex
				);
		...
		private static Boolean CheckSourceObject( Type interfaceType, Type sourceType )
		{
			// Check Properties
			foreach( PropertyInfo prop in interfaceType.GetProperties() )
			{
				...
				for( Int32 i = 0; i < origParams.Length; i++ )
				{
			...
			foreach( MethodInfo method in interfaceType.GetMethods() )
			{
				...
				for( Int32 i = 0; i < iParams.Length; i++ )
				{
		...
		/// <param name="sourceType">Type of the source.</param>
		/// <returns></returns>
		private static CodeCompileUnit GenProxyClass( Type interfaceType, Type sourceType )
		{
		...
		/// <param name="interfaceType">Type of the interface.</param>
		/// <param name="sourceType">Type of the source.</param>
		private static void GenerateProxyClassEvents
		(
			...
			Type sourceType )
		{
			foreach( EventInfo eventInfo in interfaceType.GetEvents() )
			{
				...
				( new CodeVariableDeclarationStatement( eventInfo.EventHandlerType,
																				"eventDelegate",
